...
export const OGGeneratedPreview: React.FC<OGGeneratedPreviewProps> = ({
  generateTags,
  showPreview,
  labels,
}) => {
  ...
  const handleDownload = () => {
    const tags = generateTags();
    downloadTagsAsHtml(tags, 'opengraph-tags.html');
  };
          ...
          <div className="bg-gray-900/50 rounded-lg p-4 overflow-x-auto max-h-96 overflow-y-auto">
            <pre className="text-sm text-gray-300 whitespace-pre-wrap font-mono">
              {generateTags().map((tag, index) => (
                <div key={`og-tag-${index}-${tag.slice(0, 20)}`} className="mb-1">
                  {tag}
                </div>
              ))}
            </pre>
