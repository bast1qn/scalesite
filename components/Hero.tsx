...
// ✅ PHASE 2: Pixel-perfect icon sizing (18px = 1.125rem, consistent with 4px scale)
const LightningIcon = () => (
 ...
 *
 * Features:
 * - Perfect vertical/horizontal centering (inline-flex)
 * - Consistent sizing (18px = 1.125rem, aligned to 4px scale)
  ...
  return (
    <span
      ...
      style={{
        ...
        minWidth: '1.125rem', // 18px - consistent with 4px scale
        minHeight: '1.125rem', // 18px - consistent with 4px scale
      }}
    >
      {children}
    </span>
...
const CleanButton = ({
  ...
  const baseStyle = variant === 'primary'
    ...
    : 'px-8 py-4 text-slate-700 dark:text-slate-300 font-semibold rounded-2xl border border-slate-200/60 dark:border-slate-700/60 hover:border-primary-400 dark:hover:border-primary-500 hover:bg-slate-50 dark:hover:bg-slate-800 transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] focus:ring-2 focus:ring-primary-500/50 disabled:opacity-50 disabled:cursor-not-allowed min-h-11';
  ...
  return (
    <section ref={heroRef} className="relative min-h-[100vh] flex items-center justify-center pt-24 pb-12 overflow-hidden">
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        {/* Headline - Mobile → Desktop: text-5xl → text-6xl (consistent with spec) */}
        <h1
          className={`font-display text-5xl sm:text-6xl font-bold text-slate-900 dark:text-white leading-tight tracking-tight mb-8 transition-all duration-300 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}
      ...
      >
        ...
        >
          <div className="relative w-7 h-12 rounded-full border-2 border-slate-300 dark:border-slate-700 flex items-start justify-center p-2.5 group-hover:border-primary-400 dark:group-hover:border-violet-500 transition-colors duration-300 bg-white/60 dark:bg-slate-800/60 backdrop-blur-sm min-h-11">
